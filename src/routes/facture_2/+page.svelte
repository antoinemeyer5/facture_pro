<script lang="ts">
    ...
    let tasks: any[] = $state([]);
    let new_task: any = $state({ de: null, du: null, c: null, t: null });
    function add_task() {
        if (
            new_task.de != null &&
            new_task.du != null &&
            ...
            new_task = { de: null, du: null, c: null, t: null };
        }
    }
    ...
    let tva_10 = $derived(
        tasks.reduce((acc, curr) => {
            if (curr.t == "10%") acc += curr.du * curr.c * 0.1;
            return acc;
        }, 0),
    );
    let tva_20 = $derived(
        tasks.reduce((acc, curr) => {
            if (curr.t == "20%") acc += curr.du * curr.c * 0.2;
            return acc;
        }, 0),
    );
            ...
            <tbody>
                ...
                <tr>
                    <td
                        ><input
                            type="text"
                            placeholder="produit, ligne 1, ligne 2"
